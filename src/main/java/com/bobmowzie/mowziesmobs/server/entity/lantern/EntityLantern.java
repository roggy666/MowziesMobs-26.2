    ...
    public void tick() {
        ...
        if (getAnimation() == PUFF_ANIMATION && getAnimationTick() == 7) {
            ...
            if (level().isClientSide()) {
                ...
                for (int i = 0; i < 8; i++) {
                    AdvancedParticleBase.spawnParticle(level(), ParticleHandler.PIXEL, getX(), getY() + 0.3, getZ(), -getDeltaMovement().x() * 0.2 + 0.2 * (random.nextFloat() - 0.5), -getDeltaMovement().y() * 0.2 + 0.1 * (random.nextFloat() - 0.5), -getDeltaMovement().z() * 0.2 + 0.2 * (random.nextFloat() - 0.5), true, 0, 0, 0, 0, 4f, 163d / 256d, 247d / 256d, 74d / 256d, 1, 0.9, 17 + random.nextFloat() * 10, true, true, new ParticleComponent[] {
                ...
                if (getMoveHelperController() != null && getMoveHelperController().isMovingTo()) {
...
//        if (getAnimation() == NO_ANIMATION) AnimationHandler.INSTANCE.sendAnimationMessage(this, DIE_ANIMATION);
    ...
    public void travel(Vec3 movement)
    {
        if (this.isInWater()) {
            ...
            BlockPos ground = BlockPos.containing(this.getX(), this.getBoundingBox().minY - 1.0D, this.getZ());
            float f = 0.91F;
            if (this.onGround()) {
