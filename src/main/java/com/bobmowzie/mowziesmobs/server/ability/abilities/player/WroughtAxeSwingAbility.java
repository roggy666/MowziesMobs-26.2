...
public class WroughtAxeSwingAbility extends PlayerAbility {
    ...
    public void start() {
        ...
        if (!getUser().level().isClientSide()) {
            EntityAxeAttack axeAttack = new EntityAxeAttack(EntityHandler.AXE_ATTACK, getUser().level(), getUser(), false);
            axeAttack.snapTo(getUser().getX(), getUser().getY(), getUser().getZ(), getUser().getYRot(), getUser().getXRot());
            getUser().level().addFreshEntity(axeAttack);
            this.axeAttack = axeAttack;
        }
        else {
            boolean handSide = getUser().getMainArm() == HumanoidArm.RIGHT;
            playAnimation("axe_swing_start_" + (handSide ? "right" : "left"), GeckoPlayer.Perspective.THIRD_PERSON, LoopType.PLAY_ONCE);
            playAnimation("axe_swing_start", GeckoPlayer.Perspective.FIRST_PERSON, LoopType.PLAY_ONCE);
